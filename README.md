...
英文：1级标题 -> 2级标题 -> 3级标题<br/>
中文：1级标题 -> 2级标题 -> 3级标题<br/>
中英文混合：同上 
...
- 如果下一行没有与上一行相隔一行，github会把两行合为一行，**故行与行之间必须空隔一行**

- github页面不能显示换行，使用Ctrl+Shift+K把内容代码块化即可

- **换行无空隔可以**

  添加br>/、软换行Shift+Enter、硬换行空格+空格+Shift+Enter          `其中硬换行在github页面可以显示行与行无间隔，而软换行不可以只是在Typora显示无间隔`

- 换行无空隔看起来页面不友好，一般下一行是个人理解，可使用**无序列表代替**换行无空隔

- 2级标题下的内容使用**引用**，每一行内容需要补充使用代码化
...
- 可以不使用2级标题，把2级标题改为加粗字体，2级标题的内容**就可不用引用**

- 不可滥用标题

# 算法方法论

交换位置<br/>双指针  数组:int i=0;int k=arr.length-1;i、k是该数组的双指针    双指针可以左0右n-1,也可左0右0<br/>
辅助栈 <br/>
辅助队列 辅助链表<br/>
一次次加和一次性加<br/>
数组先排好序<br/>
善用for(int i:arr){}<br/>
根据要求找出满足条件<br/>
多少种可能性的题目一般都有递推性质，即f(n)和f(n−1)…f(1)之间是有联系的 <br/>
二进制算法题,按每个位之间的关系来设计算法<br/>
辅助辅助辅助，而for、while<br/>
递归dfs(TreeNode left,TreeNode right)，丢两个去比较，而不是只丢一个，然后使用root.left和root.right去比较<br/>
...
按照从左到右、从上到下、从右到左、从下到上遍历，使用while(true)循环当所有方格被遍历时退出循环，其中int l=0,r=matrix[0].length-1,t=0,b=matrix.length-1作为矩阵左、右、上、下边界，每遍历完最顶行则t++，该行不再使用、每遍历完最右列、每遍历最底行、每遍历最左列
